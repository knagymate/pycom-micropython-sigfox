...
STATIC mp_obj_t mod_http_client_init(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
	...
	const mp_arg_t mod_http_client_init_args[] = {
			...
			{ MP_QSTR_callback,                MP_ARG_INT  | MP_ARG_KW_ONLY, {.u_obj = MP_OBJ_NULL} },
	...
	if (args[3].u_obj == MP_OBJ_NULL) {
		...
		mp_obj_t *sec;
		mp_obj_get_array_fixed_n(args[3].u_obj, 2, &sec);

		// Get user and pass from tuple
		user = mp_obj_str_get_str(sec[0]);
		pass = mp_obj_str_get_str(sec[1]);
...
STATIC mp_obj_t mod_http_client_send_request(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
	...
	if(client_config.url == NULL) {
		nlr_raise(mp_obj_new_exception_msg(&mp_type_RuntimeError, "Client was not initialized!"));
	}

	// Set Request body
	if (args[0].u_obj != MP_OBJ_NULL) {
		esp_http_client_set_post_field(client_obj, mp_obj_str_get_str(args[0].u_obj), strlen(mp_obj_str_get_str(args[0].u_obj)));
	}

	// Set Content-Type Header
	esp_http_client_set_header(client_obj, "Content-Type", mod_http_mediatype[args[1].u_int]);
...
//STATIC MP_DEFINE_CONST_FUN_OBJ_0(mod_http_client_send_request_obj, mod_http_client_send_request);
...
// Gets or sets the Client method
STATIC mp_obj_t mod_http_client_method(mp_uint_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {

	const mp_arg_t mod_http_client_callback_args[] = {
			{ MP_QSTR_method,                MP_ARG_INT, {.u_int = -1}},
	};
	mp_arg_val_t args[MP_ARRAY_SIZE(mod_http_client_callback_args)];
	mp_arg_parse_all(n_args, pos_args, kw_args, MP_ARRAY_SIZE(args), mod_http_client_callback_args, args);
	...
	if(args[0].u_int != -1) {
		...
		// Init client
		if(client_config.url != NULL) {
			esp_http_client_set_method(client_obj, client_config.method);
		}
		return mp_const_none;
	}
	else {
		// Get and return actual method
		int method;
		if(client_config.method == HTTP_METHOD_GET) {
			method = MOD_HTTP_GET;
		}
		if(client_config.method == HTTP_METHOD_POST) {
			method = MOD_HTTP_POST;
		}
		if(client_config.method == HTTP_METHOD_PUT) {
			method = MOD_HTTP_PUT;
		}
		if(client_config.method == HTTP_METHOD_DELETE) {
			method = MOD_HTTP_DELETE;
		}
		return mp_obj_new_int(method);
	}
}

STATIC MP_DEFINE_CONST_FUN_OBJ_KW(mod_http_client_method_obj, 0, mod_http_client_method);
...
STATIC const mp_map_elem_t mod_http_client_globals_table[] = {
		...
		{ MP_OBJ_NEW_QSTR(MP_QSTR_method),                          (mp_obj_t)&mod_http_client_method_obj },
};
